...
scroll.CanvasSize = UDim2.new(0, 0, 0, 300) -- Taille fixe pour le scroll
...
sliderBack.Size = UDim2.new(0, 250, 0, 8)
sliderBack.Position = UDim2.new(0.5, -125, 0, 75)
...
btnAA.Size = UDim2.new(0, 90, 0, 25)
btnAA.Position = UDim2.new(0, 10, 1, -30)
...
-- 4. LOGIQUE DU SLIDER (Correction Bug de Drag)
local sliding = false
...
local function updateSlider(input)
    ...
    walkSpeedValue = math.floor(16 + (percentage * 234)) -- Vitesse de 16 à 250
...
sliderBtn.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        sliding = true
        scroll.ScrollingEnabled = false -- Désactive le scroll pour ne pas gêner
    end
end)
...
UserInputService.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        sliding = false
        scroll.ScrollingEnabled = true -- Réactive le scroll
    end
end)
...
-- Appliquer la vitesse en boucle (Force la vitesse même si le jeu essaie de la reset)
...
-- 5. LOGIQUE BOUTONS & DRAG FENÊTRE
